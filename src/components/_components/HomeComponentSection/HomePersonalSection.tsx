...
export default function HomeAndPersonalSection() {
  const [homeAndPersonalProducts, setHomeAndPersonalProducts] =
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        ...
        setHomeAndPersonalProducts(products);
      } catch (error) {
  ...
  if (!homeAndPersonalProducts?.items?.length) return null;
  ...
  return (
    <div className="relative space-y-4 overflow-hidden py-2">
      ...
      <div className="relative mx-auto w-full max-w-screen-xl">
        ...
        >
          <CarouselContent className="flex">
            {homeAndPersonalProducts.items.map((product) => (
              <CarouselItem
