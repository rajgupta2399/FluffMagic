...
export default function OrthopaedicSection() {
  const [orthopaedicProducts, setOrthopaedicProducts] =
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        ...
        setOrthopaedicProducts(products);
      } catch (error) {
  ...
  if (!orthopaedicProducts?.items?.length) return null;
  ...
  return (
    <div className="relative space-y-4 overflow-hidden py-2">
      ...
      <div className="relative mx-auto w-full max-w-screen-xl">
        ...
        >
          <CarouselContent className="flex">
            {orthopaedicProducts.items.map((product) => (
              <CarouselItem
